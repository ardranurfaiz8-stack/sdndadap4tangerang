...
<div class="card" style="margin-top:1.25rem;">
    <div class="card-header" style="flex-wrap:wrap;gap:.75rem;">
        <span class="card-title">📋 Data Absensi Guru</span>
        <form method="GET" action="{{ route('kepala-sekolah.absen-guru.index') }}" style="display:flex;gap:.5rem;align-items:center;flex-wrap:wrap;">
            <input type="date" name="tanggal" class="form-control" value="{{ $today }}" onchange="this.form.submit()" style="width:170px;">
            <input type="text" name="search" class="form-control" placeholder="Cari nama guru..." value="{{ request('search') }}" style="width:180px;">
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><strong>{{ $a->guru->nama ?? '-' }}</strong></td>
                        <td class="text-muted" style="font-family:monospace;font-size:12px;">{{ $a->guru->nip ?? '-' }}</td>
