        ...
        <div style="display:flex;gap:0.5rem;align-items:center;flex-wrap:wrap;">
            <form method="GET" action="{{ route('admin.rekap.index') }}" id="filterForm"
                ...
                <select name="bulan" onchange="document.getElementById('filterForm').submit()"
                    style="padding:0.5rem 0.75rem;border:1.5px solid var(--gray-200);border-radius:10px;font-family:inherit;font-size:0.83rem;outline:none;">
                    @foreach(['Januari'=>1,'Februari'=>2,'Maret'=>3,'April'=>4,'Mei'=>5,'Juni'=>6,'Juli'=>7,'Agustus'=>8,'September'=>9,'Oktober'=>10,'November'=>11,'Desember'=>12] as $nm => $n)
                        <option value="{{ $n }}" {{ request('bulan', date('n')) == $n ? 'selected' : '' }}>{{ $nm }}</option>
                    @endforeach
                </select>
            ...
            <a href="{{ route('admin.rekap.export', request()->all()) }}"
                class="btn btn-primary" style="padding:0.6rem 1.5rem;font-size:0.88rem;">
                🖨️ Export
            </a>
        </div>
    </div>
...
<div class="card">
    <div class="card-header">
        ...
        <span style="font-size:0.78rem;color:var(--gray-500);">
            {{ ['1'=>'Januari','2'=>'Februari','3'=>'Maret','4'=>'April','5'=>'Mei','6'=>'Juni','7'=>'Juli','8'=>'Agustus','9'=>'September','10'=>'Oktober','11'=>'November','12'=>'Desember'][request('bulan', date('n'))] }}
